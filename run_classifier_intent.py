...
#ying: The processor for intent classifier
class IntentProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    input_lines = self._read_lines(
        os.path.join(data_dir,
                     "sentences.train.in"))
    output_lines = self._read_lines(
        os.path.join(data_dir,
                     "sentences.train.out"))

    # set label list according to training labels
    self.label_list = set(output_lines)
    ...
    for (i, line) in enumerate(input_lines):
      ...
      text_b = None
      label = tokenization.convert_to_unicode(output_lines[i])
      examples.append(
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    if FLAGS.label_smoothing is not None:
        ...
        bce = tf.keras.losses.BinaryCrossentropy(label_smoothing=FLAGS.label_smoothing)
        loss = bce(one_hot_labels, logits)
    else:
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        if per_example_loss is None:
            loss = tf.metrics.accuracy(labels=label_ids, predictions=predictions, weights=is_real_example) # dummy
        else:
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    eval(os.path.join(FLAGS.data_dir, "sentences.test.out"),
         output_predict_file, FLAGS.output_dir+'/label_ids.txt',
         oos_id=oos_id)
